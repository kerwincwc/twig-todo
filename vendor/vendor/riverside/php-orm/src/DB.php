...
class DB
{
     ...
     * @throws \Exception
     */
    public function find($value=1,$pk=null): ?array
    {
        $pk = !is_null($pk)?$pk:$this->primary_key;
        $this->where($this->table . '.' . $pk, $value);
        $this->limit(1, 0);
    ...
    protected function mount(): DB
    {
        ...
        if (!is_file(self::$config) AND !is_null( self::$config ))
        {
            throw new \Exception("File '".self::$config."' not found.");
        }
        if( is_null(self::$config))
        {
            $database = include $GLOBALS['_config']['basedir']."/app/config/database.php";
        } else {
